...
async function main() {
  ...
  const alice = await Chatter.deploy("Alice")
  const bob = await Chatter.deploy("Bob")
  ...
  await alice.send("Hello Bob!", bob.address)
  //await alice.send("Hello Greeter!", greeter.address)
}
